...
- **Throttling (`throttling.py`, #830):** every call funnels through `Request._issue`, which acquires the process-wide `RateLimiter` (shared across the worker pools), posts with a timeout, and retries transient refusals with exponential backoff + jitter. Polygon signals throttling in three shapes and all three are retried: HTTP 429/408/5xx, a non-JSON body (HTML error page) where JSON was promised, and a well-formed `FAILED` whose `comment` matches `throttling._RETRYABLE_COMMENT_RE`. Each refusal also *penalizes* the limiter (doubles the spacing, up to `RBX_POLYGON_MAX_INTERVAL`), decaying back after 10 clean responses. Genuine `FAILED`s return on the first attempt so callers still raise `PolygonRequestFailedException` with Polygon's wording -- and so does an exhausted retryable `FAILED`, which is why `_issue` returns `last_response` rather than exiting. Tunables: `RBX_POLYGON_MIN_INTERVAL`/`MAX_INTERVAL`/`MAX_RETRIES`/`TIMEOUT`
...
- **Submissions are amalgamated, not copied.** DOMjudge builds a jury solution exactly as it builds a contestant's -- from one file -- so `_solution_content` runs every C/C++ solution through [Source Amalgamation](#source-amalgamation-dependenciesamalgamationpy). Deliberately **without `extra_roots`**, unlike the MOJ packager: resolution stays natural (relative to the including file), so the builtin headers are out of a solution's reach. That is the point -- a solution is contestant-like code, and `rbx.h` reads a vars file the judge lacks, so inlining it would trade a compile error for a runtime one. Note `header.get_header()` is the *relative* path `rbx.h`, so passing its `.parent` as an extra root silently adds the **package root** as an include root; that accident is what an earlier revision of this code (and its test) leaned on. Other languages have no amalgamator: the multi-file guard uses `deps_graph.expand(solution)` **unfiltered by dependency kind** (an interpreted language reports imports as EXECUTION, not COMPILATION, so filtering on COMPILATION as MOJ does would let a Python solution importing a sibling through as a lone unrunnable file) and refuses with the deps named. Contrast the checker, which is *flattened* (N files in `output_validators/`, which DOMjudge compiles together) rather than amalgamated.
...
- **Time limits live in `conf`, never in `tl`.** MOJ *measures* the TL from `sols/good`, so no `tl` is emitted; `conf` carries `MEMLIMITMB` (peak RSS, replacing the legacy `ULIMITS[-v]`), `ULIMITS[-f]` and the time-limit knobs. **`ULIMITS[-f]` is a fixed 100 MiB, not the problem's `outputLimit`** -- MOJ applies it to the compile step too, so a tight output limit made the linker fail and every submission come back `Compilation Error`; the trade is that MOJ no longer enforces `outputLimit` (rbx still does, locally). By default `rbx package moj` **pins** the limits estimated by `rbx time -p moj` through `TLOVERRIDE[default]` plus a `TLOVERRIDE[<lang>]` for every language off that default: MOJ applies `TLOVERRIDE` *after* the `TLMOD`/calibration arithmetic, both when judging and everywhere it displays a limit, so calibration can no longer move them. (The old zero-slope `TLMOD[calibrafactor]=<base - 0.02>+0` plus `TLMOD[<lang>.sum]` trick is gone.) `CALIBRATIONTL` still goes with them at max(largest pinned limit, 5s, `inferenceTimeout`), so calibration -- which still has to run, since mojtools refuses to judge without `tl` -- is never tighter than what the problem, or rbx's own estimation, allowed a solution. Without that profile packaging refuses; `--calibrate` instead emits `TLMOD[calibrafactor]=<acToTimeLimit>`, no override, and lets the judge decide. See `moj/timing.py`.
...
- **`.moj-meta.json`.** Only the *content* fields the server accepts from a tar: `display_title` (always) and `languages` (the languages `env.rbx.yml` declares, i.e. the ones the package ships `scripts/<lang>/` and a `TLOVERRIDE` for; `--calibrate` warns about those with no accepted solution, since MOJ can only calibrate those). The *access* fields (`public`/`public_at`/`owner`) are never written -- they are ignored from a tar anyway, and `public` is fail-closed server-side.
- **Markdown statements.** `task_types()` is `[BATCH]`; `statement_types()` is the default `[PDF]` for every package a setter builds, and returns nothing only for a probe package, which ships no statement (it names the *output* type, and v2 emits only pdf/tex/md -- returning the source type `rbxTeX` fails the build, and TeX/Markdown output would skip `render.compile_pdf`, where externalization and demacro actually run) while `statement_export_params()` forces externalize+demacro, so the packager consumes [the export bundle](#statement-export-bundle-rbxboxstatementsexportpy) and converts each block TeX -> Markdown with pandoc -- which is the target dialect, since MOJ renders statements by running pandoc over them. `docs/enunciado.md` carries no title (MOJ injects the `<h1>` from `display_title`), the mandatory `## Entrada`/`## Saída`, and neither an examples section nor a fence (both trip `validate-problem.sh`). Sample explanations become `docs/notes/<sample>.md`, paired to the test **by name**. `--language` selects the statement, and `display_title` resolves from the same one. A package with no statement keeps the `DUMMY_STATEMENT` fallback. **`SubtreeLayout`'s first production consumer**; PDF figures are rasterized with poppler, so packaging refuses without it.
- `MojLanguageExtension` (key `moj`) mirrors `BocaLanguageExtension`: `languages` + required `template` + optional `flags`.
- **`--reference-only`.** Ships only the model solution, dropping the rest. Calibration runs everything the package ships, so this is what makes an upload cheap to iterate on -- at the cost of the judge never verifying the dropped solutions, which the packager warns about.
- **`--upload` calibrates the whole park.** `moj/upload.py` queues the calibration with `moj calibrate <id> --all-judges`, where the *runner*'s probe upload (`runners/moj/runner.py`) asks for the cheap global request. The judges are not identical machines and MOJ publishes the TL as the **max across the judges that calibrated**, so a package measured on one is judged, everywhere else, against a limit nothing measured there. `--all-judges` and never a `--hosts` list rbx assembles: the CLI expands the flag against the park it queries at that moment, so a changed inventory is never ours to know. Targeting resolves that host list *before* it queues anything, which is a way to fail the global request does not have -- so `_calibrate_everywhere` falls back to the global request, because the package is already uploaded by then and mojtools will not judge one with no `tl`.
- **Probe packages.** `MojPackager(probe=ProbePackage(...))` builds the throwaway package the MOJ *runner* (`rbx/box/runners/moj/`) uploads to measure timings on the judge: model solution only, the `TLOVERRIDE` block the run asked for (one cap while estimating, one per language group while validating), every testrunnable language whitelisted, the `STOPWHEN_*` bits its caller's abort predicate translates to (`halt_on`) and no statement build -- so it can be built without `run_packager`, whose full local verification run is the thing a remote runner exists to avoid.
