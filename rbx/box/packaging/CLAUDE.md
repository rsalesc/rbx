...
- **Submissions are amalgamated, not copied.** DOMjudge builds a jury solution exactly as it builds a contestant's -- one file, without the headers rbx injects beside a source -- so `_solution_content` runs every C/C++ solution through [Source Amalgamation](#source-amalgamation-dependenciesamalgamationpy) with `extra_roots=[testlib dir, rbx.h dir]`, which is what makes a solution that includes `rbx.h` or a local header compile on the judge. Other languages have no amalgamator: the multi-file guard uses `deps_graph.expand(solution)` **unfiltered by dependency kind** (an interpreted language reports imports as EXECUTION, not COMPILATION, so filtering on COMPILATION as MOJ does would let a Python solution importing a sibling through as a lone unrunnable file) and refuses with the deps named. Contrast the checker, which is *flattened* (N files in `output_validators/`, which DOMjudge compiles together) rather than amalgamated.
