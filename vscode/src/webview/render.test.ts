...
const CARD_CASE = row({
  ...
  card: {
    ...
    // This package declares no visualizer, so the card shows no lazy buttons.
    visualizers: { input: false, output: false },
    origins: [
...
test('the run card offers a visualize button per declared visualizer', () => {
  // These run rbx, so they appear only where `problem.rbx.yml` declares
  // something to run -- a button whose only outcome is "no visualizer
  // declared" is worse than no button.
  const declared = {
    ...CARD_CASE,
    card: { ...CARD_CASE.card!, visualizers: { input: true, output: true } },
  };
  const html = renderCard(model([MAIN, MAIN_GROUP, declared]), state({ selected: declared.id }));
  assert.ok(html.includes('data-action="rbx.visualizeTest"'), html);
  assert.ok(html.includes('data-action="rbx.visualizeSolutionOutput"'), html);
  assert.ok(html.includes('class="card-channel lazy"'), html);
  // On its own line, below the channel bar: five buttons wrap in a ~300px
  // sidebar, and wrapping is the reflow `cardChannels` exists to avoid.
  assert.ok(html.includes('<span class="card-channels-label">visualization</span>'), html);
});

test('the run card offers only the channels that are declared', () => {
  const outputOnly = {
    ...CARD_CASE,
    card: { ...CARD_CASE.card!, visualizers: { input: false, output: true } },
  };
  const html = renderCard(
    model([MAIN, MAIN_GROUP, outputOnly]),
    state({ selected: outputOnly.id }),
  );
  assert.ok(!html.includes('data-action="rbx.visualizeTest"'), html);
  assert.ok(html.includes('data-action="rbx.visualizeSolutionOutput"'), html);
});

test('a package declaring no visualizer gets no visualization row', () => {
  // CARD_CASE declares none, so the existing out/err/log bar stands alone.
  const html = renderCard(
    model([MAIN, MAIN_GROUP, CARD_CASE]),
    state({ selected: CARD_CASE.id }),
  );
  assert.ok(!html.includes('card-channels-label'), html);
  assert.ok(html.includes('data-action="out"'), html);
});
