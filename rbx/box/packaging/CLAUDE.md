...
- **`--main-solution-only`.** Ships only the model solution, dropping the rest. Calibration runs everything the package ships, so this is what makes an upload cheap to iterate on -- at the cost of the judge never verifying the dropped solutions, which the packager warns about.
