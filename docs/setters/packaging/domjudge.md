...
Interactive (**communication**) problems are packaged too. DOMjudge has a single mode
for interaction: the output validator stops being a compare script and becomes the
problem's *run* script, wrapped so that it and the submission talk to each other over a
pair of pipes. {{rbx}} declares that with `validation: custom interactive` in
`problem.yaml` and ships your interactor as that validator, so the interaction DOMjudge
drives is the one {{rbx}} drives locally, and the verdict your interactor reports is the
verdict DOMjudge records.

If your interactor is a [legacy interactor](../grading/interactors.md) — one that writes
an output for a separate checker to grade — both programs are shipped, and {{rbx}} also
writes a small `build`/`run` pair under `output_validators/` that chains them: the
interactor runs against the submission, and its output is then handed to the checker
along with the answer file. An interaction that did not accept skips the checker, so it
keeps its own verdict.

Interactors, like checkers, must be written in C++ (testlib).

!!! note
    The answer files under `data/` are the outputs {{rbx}} generated by running the main
    solution through the interactor. A modern interactor never reads them; a legacy
    checker does, which is exactly what they are there for.
